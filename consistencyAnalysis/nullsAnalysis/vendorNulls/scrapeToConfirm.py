...
alreadyAnalyzed = set()
f1 = open('./vendorNullsCurrentStatusFromWeb.csv', 'rb')
for l in f1:
    l = l.decode().replace('\n', '').rsplit(';')
    alreadyAnalyzed.add(l[0])


f = open('./vendorNullsCurrentStatus.csv', 'rb')
for line in f:
    ...
    if cve in alreadyAnalyzed:
        ...
        out1 = ";".join(line)+";"
        with open('./vendorNullsCurrentStatusFromWeb.csv', 'a') as foo1:
            foo1.write(out1 + '\n')
        continue
    ...
    for i in range(len(changeHistory)):
        if "CPE" in changeHistory[i].text:
    ...
    with open('./vendorNullsCurrentStatusFromWeb.csv', 'a') as foo:
        foo.write(out+'\n')
    print(out)