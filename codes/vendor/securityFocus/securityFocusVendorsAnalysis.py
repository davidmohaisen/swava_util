...
# vendorInconsistentChainsFromNVD = open('./vendorChains-Final.xlsx', 'rb')
...
print("------------------------------ security Focus results ------------------------")
url = 'https://www.securityfocus.com/bid'
html = urlopen(url).read()

soup = BeautifulSoup(html, 'html.parser')
text = soup.get_text().rsplit('\n')
print("Vendors as extracted from SecurityFocus: ", text)
...
for i in range(len(text)):
    ...
    if string == "":
        continue
    if string == "selectvendor" and start == 0:
        start = 1
        continue
    if string == "title":
        end = 1
...
with open('./output/SecurityFocusVendors.json', 'w') as f0:
    f0.write(x+'\n')
dbProductCount = 0
